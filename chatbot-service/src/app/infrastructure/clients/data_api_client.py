...
class DataApiClient:
    """HTTP client for calling data-api endpoints."""
    ...
    async def get_knowledge_base(self, kb_id: str) -> Dict[str, Any]:
        ...
        """
        url = f"{self.base_url}/api/v1/kb/{kb_id}"
        ...
        try:
            ...
            logger.error(f"Data API get KB error: {e}")
            raise ExternalServiceError(
                "Data API",
                f"Get KB failed with status {e.response.status_code}",
                {"kb_id": kb_id, "error": str(e)}
            )
        except Exception as e:
            logger.error(f"Data API error: {e}")
