...
def get_op_num_threads(env_name: str) -> int:
    """Stub: read int from env var, default -1 (= onnxruntime auto)."""
    val = _os.getenv(env_name, None)
    if val is None:
        return -1
    try:
        return int(val)
...
class TSRUnet:
    def __init__(self, config: Dict):
        ...
        config["intra_op_num_threads"] = get_op_num_threads("PARSER_INTRA_OP_NUM_THREADS")
        config["inter_op_num_threads"] = get_op_num_threads("PARSER_INTER_OP_NUM_THREADS")
